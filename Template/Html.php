...
class Html
{
    ...
    public function processNode(HtmlToken $n) {
        if ($n->name == 'text') {
            $nows = count($this->nows_stack) > 0 ? end($this->nows_stack) : false;
            if ($nows) {
                $n->source = trim($n->source);
            }
            return;
        }
        ...
        if ($n->hasAttribute('fx:nows')) {
            $nows = $n->getAttribute('fx:nows') !== 'false';
            $this->nows_stack []= $nows;
            $n->setPayload('nows', $nows);
            $n->removeAttribute('fx:nows');
            
            $n->wrap(
                '{nows value="' . var_export($nows,1). '"}',
                '{/nows}'
            );
        }
    ...
    public function afterProcessNode(HtmlToken $n) {
        if (!is_null($n->getPayload('nows'))) {
    ...
    public function transformToFloxim()
    {
        ...
        $tree->apply(array($this, 'processNode'), array($this, 'afterProcessNode'));
        $res = $tree->serialize();
        return $res;
    }
