...
class Suitable
{
    ...
    public function suit(System\Collection $infoblocks, $layout_id, $layout_style_id)
    {
        ...
        foreach ($infoblocks as $ib) {
            ...
            if ($ib->getPropInherited('action') === 'record' && $ib['scope_type'] === 'infoblock_pages') {
                $list_infoblock = fx::data('infoblock', $ib['scope_infoblock_id']);
                if ($list_infoblock) {
                    $list_infoblock_visual = $list_infoblock->getVisual();
                    if ($list_infoblock_visual && $ib_visual['layout_id'] == $list_infoblock_visual['layout_id']) {
                        $ib_visual['area'] = $list_infoblock_visual['area'];
                        $ib_visual['priority'] = $list_infoblock_visual['priority'] + 0.01;
                    }
                }
            }
            if (!$ib_visual['area']) {
                ...
                elseif (preg_match("~^column_\d+~", $old_area)) {
            ...
            foreach ($controller_templates as $c_tpl) {
                if ($c_tpl['full_id'] === $old_template) {
                    ...
                    $ib_visual['template_visual'] = $ib_visual->copyParams($ib->getPropInherited('visual.template_visual', $source_layout_id));
            ...
            if ($old_wrapper){
                ...
                if (count($avail_wrappers)) {
                    ...
                    $ib_visual['wrapper_visual'] = $ib_visual->copyParams($ib->getPropInherited('visual.wrapper_visual', $source_layout_id));
                }
            }
