...
class fx_template_suitable {
    ...
    public function suit(fx_collection $infoblocks, $layout_id) {
        ...
        // seems to be second call of ::get_areas(), can be cached or reused
        ...
        foreach ($infoblocks as $ib) {
            ...
            $controller_templates = $ib_controller->get_available_templates($layout['keyword'], $area_meta);
            ...
            $old_wrapper = $ib->get_prop_inherited('visual.wrapper', $source_layout_id);
            if ($old_wrapper && $area_meta) {
                //fx::log('had wrp', $old_wrapper, $area_meta, self::parse_area_suit_prop($area_meta['suit']));
                $area_suit = self::parse_area_suit_prop($area_meta['suit']);
                if ($area_suit['default_wrapper']) {
                    $ib_visual['wrapper'] = $area_suit['default_wrapper'][0];
                    $ib_visual['wrapper_visual'] = $ib->get_prop_inherited('visual.wrapper_visual', $source_layout_id);
                }
            }
    ...
    protected function _adjust_layout_visual($layout_ib, $layout_id, $source_layout_id) {
        ...
        if ($is_root_layout && $source_layout_id) {
            $root_layout_ib = $layout_ib->get_root_infoblock();
            if ($root_layout_ib->get_visual($layout_id)->get('is_stub')) {
                $this->_adjust_layout_visual($root_layout_ib, $layout_id, $source_layout_id);
            }
        }
        ...
        if ($source_layout_id) {
            ...
            if (!$is_root_layout) {
                $source_template_params = $layout_ib->get_prop_inherited('visual.template_visual', $source_layout_id);
            }
    ...
    protected function _map_areas($old_set, $new_set) {
        ...
        foreach ($old_set as &$old_area) {
            ...
            foreach ($new_set as $new_area_id => $new_area) {
                ...
                $area_match += $this->compare_names($old_area['id'], $new_area['id']);
     ...
     */
    protected function compare_names($s1, $s2) {
